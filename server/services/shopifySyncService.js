...
const { Store, ProductShop, ProductVariantShop } = db;
const productService = new ProductService();

class ShopifySyncService {
  /**
   * Create product on single Shopify store with images
   */
  async syncProductToStore(productId, storeId) {
    ...
    try {
      ...
      // Step 2: Upload product images (from product_images table)
      if (product.images && product.images.length > 0) {
        ...
        const mediaInputs = product.images.map(img => ({
        ...
        if (imageResponse.data.productCreateMedia.mediaUserErrors?.length) {
          console.warn('Product image upload warnings:', imageResponse.data.productCreateMedia.mediaUserErrors);
        }
      }
      ...
      // Step 3: Create variants (each variant can have its own imageUrl)
      const variantsInput = product.variants.map(variant => {
        ...
        // If variant has its own imageUrl, include it
        // Note: Shopify will handle the image upload via URL
        if (variant.imageUrl) {
          variantInput.imageSrc = variant.imageUrl;
        }
      ...
      return {
        ...
        productImagesUploaded: product.images?.length || 0,
        variantsWithImages: product.variants.filter(v => v.imageUrl).length
      };
    } catch (error) {
      // Save failed sync
